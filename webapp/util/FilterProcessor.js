...
// Provides class sap.ui.model.odata.FilterProcessor
sap.ui.define([
		'sap/ui/model/Sorter',
	...
	function (Sorter, FilterProcessor, DateFormat, Log, assert, jQuery, encodeURL, CalendarType) {
		...
		/**
		 * @alias sap.ui.model.odata.FilterProcessor
		 * @namespace
		 * @public
		 */
		var FilterProcessor = function () {};
		 ...
		 * @private
		 */
		FilterProcessor.createSortParams = function (aSorters) {
			var sSortParam;
		 ...
		 * @see FilterProcessor._createFilterParams
		 ...
		 * @private
		 */
		FilterProcessor.createFilterParams = function (vFilter, oMetadata, oEntityType, sType) {
			var oFilter;
			...
			return "&$filter=" + this._createFilterParams(oFilter, oMetadata, oEntityType, sType);
		};
		 ...
		 * @private
		 */
		FilterProcessor._createFilterParams = function (vFilter, oMetadata, oEntityType, sType) {
			var that = this,
		 ...
		 */
		FilterProcessor._createUrlParamsArray = function (vParams) {
		 ...
		 * @private
		 */
		FilterProcessor._encodeURLParameters = function (mParams) {
			if (!mParams) {
				return "";
			}
			var aUrlParams = [];
		 ...
		 */
		FilterProcessor.setOrigin = function (sServiceURL, vParameters) {
			...
			if (typeof vParameters == "string") {
				...
				if (!sOrigin) {
					...
					// sanity check
					if (!sSystem || !sClient) {
						Log.warning("FilterProcessor.setOrigin: No Client or System ID given for Origin");
						return sServiceURL;
					}
		 ...
		 */
		FilterProcessor.setAnnotationOrigin = function (sAnnotationURL, vParameters) {
			...
			if (iAnnotationIndex >= 0) { // annotation path is there
				if (sAnnotationURL.indexOf("/$value", iAnnotationIndex) === -1) { // $value missing
					Log.warning("FilterProcessor.setAnnotationOrigin: Annotation url is missing $value segment.");
					...
					var sAnnotationWithOrigin = FilterProcessor.setOrigin(sAnnotationUrlBase, vParameters);
				...
				sFinalAnnotationURL = FilterProcessor.setOrigin(sAnnotationURL, vParameters);
		 ...
		 * @private
		 */
		FilterProcessor._resolveMultiFilter = function (oMultiFilter, oMetadata, oEntityType, sType) {
			var that = this,
		 ...
		 * @private
		 */
		FilterProcessor._createFilterSegment = function (sPath, oMetadata, oEntityType, sOperator, oValue1, oValue2, bCaseSensitive, sType) {
			...
			if (!bCaseSensitive && (sType === "Edm.String" || sType.toLowerCase().indexOf("string") > -1)) {
				sPath = "toupper(" + sPath + ")";
			}
		 ...
		 * @public
		 */
		FilterProcessor.formatValue = function (vValue, sType, bCaseSensitive) {
			var oDate, sValue;
		 ...
		 * @private
		 */
		FilterProcessor.parseValue = function (sValue) {
		 ...
		 * @public
		 */
		FilterProcessor.compare = function (vValue1, vValue2, bAsDecimal) {
		 ...
		 * @public
		 */
		FilterProcessor.getComparator = function (sEdmType) {
			switch (sEdmType) {
			case "Edm.Date":
			case "Edm.DateTime":
			case "Edm.DateTimeOffset":
			case "Edm.Time":
				return FilterProcessor.compare;
			case "Edm.Decimal":
			case "Edm.Int64":
		...
		FilterProcessor._normalizeKey = function (sKey) {
			return sKey.replace(rNormalizeString, fnNormalizeString).replace(rNormalizeCase, fnNormalizeCase).replace(rNormalizeBinary,
				fnNormalizeBinary);
		};
